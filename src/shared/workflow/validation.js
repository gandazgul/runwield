...
function buildMergeRepairRequest({
    ...
    return [
        ...
        `Merge path: ${mergeFailureKind === "detached_merge_conflict"
...
export async function runMechanicalValidation({
    ...
    while (true) {
        ...
        emitRunWieldSystemStatus(
            hostedSession,
            `QUICK_FIX CI failed. Dispatching ${getAgentDisplayName(AGENTS.ENGINEER, projectRoot)
        ...
        if (!completed) {
            const reason = `${getAgentDisplayName(AGENTS.ENGINEER, projectRoot)
            ...
            emitRunWieldSystemStatus(
                hostedSession,
                `${reason} Staying with ${getAgentDisplayName(AGENTS.ENGINEER, projectRoot)
...
export async function runValidationLoop({
    ...
    const pauseForExecutionContinuation = async (reason) => {
        ...
        emitRunWieldSystemStatus(
            hostedSession,
            `${reason} Staying with ${getAgentDisplayName(executionAgent, projectRoot)
            } so the user can continue the session. ` +
    ...
    while (!executionComplete && !haltReason) {
        ...
        while (!buildPasses && mechanicalAttempts < 3) {
            ...
            if (ciResult.exitCode === 0) {
                ...
                emitRunWieldSystemStatus(
                    hostedSession,
                    `Build failed. Dispatching ${getAgentDisplayName(executionAgent, projectRoot)
                    } to fix syntax/types...`,
                    true,
                    progress,
                );
                ...
                if (shouldOpenReview) {
                    ...
                    if (guidedReviewMode === "ask" && guidedRecommendation.recommended) {
                        const guidedReviewResponse = await requestInteraction(hostedSession, {
                            type: RuntimeInteractionTypes.SELECT,
                            prompt:
                                `Generate a Guided Review Explainer before code review? This uses an additional LLM call. Reasons: ${guidedRecommendation.reasons.join(", ") || "policy recommendation"
                                }.`,
                            options: [
                    ...
                    if (guidedReview.autoStart) {
                        emitRunWieldSystemStatus(
                            hostedSession,
                            `Opening code review with Guided Review generation queued (extra LLM call). Reasons: ${guidedReview.reasons.join(", ") || guidedReview.mode
                            }...`,
                        );
                    } else {
                    ...
                    if (humanReview.approved) {
                        ...
                        emitRunWieldSystemStatus(
                            hostedSession,
                            `User code review returned feedback. Sending feedback back to ${getAgentDisplayName(executionAgent, projectRoot)
                ...
                try {
                    ...
                    if (mergeVerificationFailure) {
                        ...
                        if (mergeRepairAttempts < maxMergeRepairAttempts) {
                            ...
                            emitRunWieldSystemStatus(
                                hostedSession,
                                `Post-merge verification found remaining merge-back work. Dispatching ${getAgentDisplayName(executionAgent, projectRoot)
                    ...
                    if (mergeRepairAttempts < maxMergeRepairAttempts) {
                        ...
                        emitRunWieldSystemStatus(
                            hostedSession,
                            `Dispatching ${getAgentDisplayName(executionAgent, projectRoot)
                        ...
                        progress = updateValidationProgress(progress, {
                            outcome: "paused",
                            message: `${getAgentDisplayName(AGENTS.ENGINEER, projectRoot)
