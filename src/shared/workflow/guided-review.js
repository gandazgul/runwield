...
export function buildGuidedReviewPrompt({ diffText, gitRef, changedFiles, planContent = "", planAttrs = {} }) {
    return `You are generating a RunWield Guided Review Explainer for validation-time human code review.\n\nReturn ONLY JSON with this shape:\n{\n  "schemaVersion": "${GUIDED_REVIEW_EXPLAINER_SCHEMA_VERSION}",\n  "title": string,\n  "intent": string,\n  "sections": [{\n    "title": string,\n    "role": "core" | "consequence" | "data_flow" | "ui_behavior" | "edge_case" | "support" | "glue",\n    "blocks": [\n      { "type": "prose", "markdown": string } |\n      { "type": "callout", "tone": string, "title": string, "markdown": string } |\n      { "type": "mermaid", "title": string, "source": string } |\n      { "type": "diff", "file": one_of_changed_files, "summary": string } |\n      { "type": "reviewCheckpoint", "markdown": string } |\n      { "type": "widget", "id": string, "entry": "index.html", "title": string, "reason": string, "html": string }\n    ]\n  }],\n  "everythingElse": [{ "file": one_of_changed_files }]\n}\n\nRules:\n- The intent is a concise summary of the problem and resulting behavior (1-2 sentences), not a description of the guide format.\n- Organize the guide into conceptual chapters: the core behavior first, its consequences next, supporting changes and glue last. Group related files across directories; keep tests beside the implementation they verify.\n- Aim for 2-6 chapters (one for a small change, at most 10). Use descriptive chapter titles rather than filenames or role labels.\n- Begin each chapter with prose explaining what changed, why, and the implications for the reviewer. This prose appears beside the chapter file list and live diffs on wide screens, and above them on narrow screens.\n- Give every diff a concise semantic summary explaining its part in the chapter. Place every changed file exactly once across chapter diffs and everythingElse. Reserve everythingElse for incidental changes.\n- Callouts, diagrams, checkpoints, and optional widgets accompany the chapter overview; they should remain readable in a narrow column.\n- Section role MUST be one of: ${
