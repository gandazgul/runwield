...
Deno.test("runPlansCommand delegates pull subcommand before list parsing", async () => {
    let delegated = false;

    await runPlansCommand(
        ["pull", "done", "--plan-server", "https://plans.example"],
        /** @type {any} */ ({
            __testDeps: {
                runPlansPullCommand: (/** @type {string[]} */ argv) => {
                    delegated = argv.join(" ") === "done --plan-server https://plans.example";
                },
                listPlans: () => {
                    throw new Error("listPlans should not be called");
...
Deno.test("runPlansCommand delegates push subcommand before list parsing", async () => {
    let delegated = false;

    await runPlansCommand(
        ["push", "done", "--plan-server", "https://plans.example"],
        /** @type {any} */ ({
            __testDeps: {
                runPlansPushCommand: (/** @type {string[]} */ argv) => {
                    delegated = argv.join(" ") === "done --plan-server https://plans.example";
                },
                listPlans: () => {
                    throw new Error("listPlans should not be called");
...
Deno.test("runPlansCommand delegates unshare subcommand before list parsing", async () => {
    let delegated = false;
    ...
    await runPlansCommand(
        ...
        /** @type {any} */ ({
            __testDeps: {
                runPlansUnshareCommand: (/** @type {string[]} */ argv) => {
                    delegated = argv.join(" ") === "done --force";
                },
                listPlans: () => {
                    throw new Error("listPlans should not be called");
                },
