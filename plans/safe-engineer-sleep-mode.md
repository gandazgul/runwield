...
complexity: "MEDIUM"
summary: "Change /sleep to mechanically export a restorable Mnemosyne backup into the active session’s ~/.wld/sessions artifact area, announce the backup path via an initial system message, invoke Engineer as the persistent root agent for follow-up questions, and update the command-owned prompt/help/tests accordingly."
affectedPaths:
...
The existing command-owned prompt must remain non-overridable. Its current conservative refinements—lossless
consolidation rules, deletion classification, manifesting, and a large-deletion approval threshold—are intentional and
must be preserved. Remove only responsibility for creating or protecting the pre-maintenance recovery backup from the
Agent prompt. The backup is for the current project's Mnemosyne collection, including memories tagged `core`; sleep mode
does not broaden its cleanup scope to unrelated project collections.
...
- `src/cmd/sleep/prompt.md` — preserve the current conservative cleanup rules, remove Agent-owned recovery backup
  creation/validation, and redirect retained manifest/post-maintenance-export/report artifacts away from the repository
  root.
...
- Preserve command-owned prompt behavior so local/home prompt-template overrides cannot weaken the mechanical backup or
  cleanup rules. The current conservative prompt rules are approved input to this feature and must not be discarded
  while synchronizing the inlined prompt.
...
- The working tree may contain unrelated user changes; implementation must not overwrite, clean, or incorporate them.
