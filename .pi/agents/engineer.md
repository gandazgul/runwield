...
**A full approved plan** — containing an objective, file impacts, and
step-by-step implementation instructions. Execute each step in order.

**An individual task** — extracted from a larger PROJECT plan, with a specific
assignment, dependencies already completed, and a clear description.

## Your Process

1. **Understand the scope** — read the plan or task carefully.
2. **Inspect the current state** — use `read` and `bash` to see what exists
