...
import { r as renderMarkdown } from './_planId__t28ln7Ig.mjs';
/* empty css                            */

function PlannotatorPlanBody({ markdown }) {
...
function readEmbeddedPlanBody(host) {
...
async function loadPlanBody(planId, host) {
...
function unmountPlannotatorPlanBody(host) {
...
async function mountPlannotatorPlanBody(host) {
...
function handlePlannotatorPlanBodyMount(event) {
  if (!(event.target instanceof HTMLElement)) return;
  const host = event.target.matches("[data-plannotator-plan-body]") ? event.target : event.target.closest("[data-plannotator-plan-body]");
  if (!host || host.dataset.plannotatorRenderer === "rendered" || host.dataset.plannotatorRenderer === "loading") {
    return;
  }
  void mountPlannotatorPlanBody(host);
}
function findPlannotatorPlanBodyHosts(node) {
  if (!(node instanceof HTMLElement)) return [];
  const hosts2 = Array.from(node.querySelectorAll("[data-plannotator-plan-body]"));
  if (node.matches("[data-plannotator-plan-body]")) hosts2.unshift(node);
  return hosts2;
}
