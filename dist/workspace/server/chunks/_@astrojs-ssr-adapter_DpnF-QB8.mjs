  ...
  if (callerInfo && context.request.method === "POST" && !actionResultAlreadySet) {
    action = {
      ...
      handler: async () => {
        ...
        ) ? removeTrailingForwardSlash$1(callerInfo.name) : callerInfo.name;
        let baseAction;
        try {
...
function findRouteToRewrite({
  ...
  if (base !== "/") {
    const isBasePathRequest = newUrl.pathname === base || newUrl.pathname === removeTrailingForwardSlash$1(base);
    if (isBasePathRequest) {
      pathname = shouldAppendSlash ? "/" : "";
    } else if (newUrl.pathname.startsWith(base)) {
      pathname = shouldAppendSlash ? appendForwardSlash(newUrl.pathname) : removeTrailingForwardSlash$1(newUrl.pathname);
  ...
  if (base !== "/" && (pathname === "" || pathname === "/") && !shouldAppendSlash) {
    newUrl.pathname = removeTrailingForwardSlash$1(base);
  } else {
...
function setOriginPathname(request, pathname, trailingSlash, buildFormat) {
  ...
  if (pathname === "/") {
    ...
    finalPathname = removeTrailingForwardSlash$1(pathname);
  }
...
class App {
  ...
  constructor(manifest, streaming = true) {
    ...
    this.#baseWithoutTrailingSlash = removeTrailingForwardSlash$1(this.#manifest.base);
  ...
  #redirectTrailingSlash(pathname) {
    ...
    if (trailingSlash === "never") {
      return removeTrailingForwardSlash$1(pathname);
    }
    return pathname;
  }
  ...
  #getDefaultStatusCode(routeData, pathname) {
    ...
    const route = removeTrailingForwardSlash$1(routeData.route);
...
function start(manifest, options) {
  ...
  const handler = async (request, handlerInfo) => {
    ...
    if (routeData) {
      ...
      const hostname2 = handlerInfo.remoteAddr?.hostname;
      Reflect.set(request, Symbol.for("astro.clientAddress"), hostname2);
